...
export async function processMessage(input: ProcessMessageInput): Promise<ProcessMessageResult> {
  ...
  try {
    ...
    // ========================================
    // FAST PATH: Fast Lane Detection (Search, Greetings, etc)
    // ========================================
    const fastLaneResult = tryFastLane(messageText, currentContext.state as ConversationState, currentContext, settings);
    
    if (fastLaneResult.matched) {
      console.log(`⚡ [FAST PATH] Fast Lane Matched: ${fastLaneResult.action}`);

      // Handle SEARCH_PRODUCTS
      if (fastLaneResult.action === 'SEARCH_PRODUCTS') {
        const query = fastLaneResult.query || messageText;

        console.log(`🔍 [FAST PATH] Executing Search: "${query}"`);
        const searchResult = await searchProducts(query, input.workspaceId);

        if (searchResult.success && searchResult.products.length > 0) {
          // Update context with identified products
          currentContext.metadata.identifiedProducts = searchResult.products;
          currentContext.state = 'CONFIRMING_PRODUCT';
          
          // Persist context
          await updateContextInDb(supabase, input.conversationId, 'CONFIRMING_PRODUCT', currentContext);
          ...
          // Send results (Generic Templates)
          // We need to render the response text too
          const responseText = settings.businessCategory === 'food'
            ...
            : `অর্ডার করতে চাইলে উপরের কার্ডের 'Order Now 🛒' বাটনে ক্লিক করুন 😊`;

          // This will be handled by the "interception" logic below if we don't return here,
          // but we want to SKIP the AI agent entirely.
          // So we re-use the rendering logic here or just fall through after setting identifiedProducts?
          // If we fall through, it still calls runAgent. We MUST return here.
          
          const mappedProducts = searchResult.products.map((p: any) => ({
            id: p.id,
            name: p.name,
            price: p.price,
          ...
          if (!input.isTestMode) {
             ...
             await sendMessage(input.pageId, input.customerPsid, responseText);
          }
      ...
      // Handle other fast lane actions (GREETING, etc)
      if (fastLaneResult.action === 'GREETING' && fastLaneResult.response) {
         // Update context if needed
         if (fastLaneResult.updatedContext) {
           currentContext = { ...currentContext, ...fastLaneResult.updatedContext };
         }
         
         await updateContextInDb(supabase, input.conversationId, fastLaneResult.newState || 'IDLE', currentContext);
         ...
         return {
           ...
           newState: fastLaneResult.newState || 'IDLE',
           updatedContext: currentContext,
         };
      }
    }
