...
export async function processMessagingEvent(
  ...
  try {
    ...
    // ========================================
    // 0. ROBUST IDEMPOTENCY LOCK (IMMEDIATE)
    // ========================================
    const eventId = generateEventId(entryId, timestamp, messageId);
    
    // EXTREMELY AGGRESSIVE: Block in-memory BEFORE any async calls
    if (processedEventIds.has(eventId)) {
      console.log(`🛡️ [IDEMPOTENCY] Blocked parallel race condition: ${eventId}`);
      return;
    }
    processedEventIds.add(eventId); // Immediate lock
    ...
    // Immediate insertion to lock the event ID in DB
    try {
    ...
    if (event.postback) {
      ...
      if (payload.startsWith('ORDER_NOW_')) {
        ...
        // Immediate visual feedback — show typing indicator right away
        const { typingOn, sendMessage, sendImage } = await import('@/lib/facebook/messenger');
        ...
        if (product) {
          ...
          if (fbPage) {
            ...
            if (conversation) {
              ...
              // STEP 1: Send product image for visual confirmation
              ...
              if (productImageUrl) {
                try {
                  ...
                  if (fbPageToken) {
                    ...
                    await sendImage(pageId, customerPsid, productImageUrl, accessToken);
                  }
                } catch (imgErr) {
                  console.warn('⚠️ [ORDER_NOW] Could not send product image:', imgErr);
                }
              }
