...
export function ProductForm({
  ...
  useEffect(() => {
    if (product) {
      ...
      // Load food specific fields
      if (product.flavors) {
        if (Array.isArray(product.flavors)) {
          setFlavors(product.flavors);
        } else if (typeof product.flavors === 'string') {
          setFlavors((product.flavors as string).split(',').map(f => f.trim()).filter(Boolean));
        }
      } else {
        setFlavors([]);
      }
  ...
  const onSubmit = async (values: ProductFormValues) => {
    try {
      ...
      const totalStock = isFood ? 99999 : finalSizeStock.reduce((sum, item) => sum + item.quantity, 0);

      if (isFood || finalSizeStock.length > 0) {
        if (!isFood) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[550px] max-h-[90vh] overflow-y-auto bg-zinc-950/95 dark:backdrop-blur-xl border-white/10 shadow-2xl p-0 gap-0">
        ...
        <Form {...form}>
            ...
            <div className="space-y-3">
              <div className="flex flex-col gap-0.5">
                <FormLabel className="text-zinc-400 uppercase text-xs font-bold tracking-wider">
                  {isFood ? 'Product Photos' : 'Product Images'}
                ...
                <p className="text-[10px] text-zinc-500 leading-tight">
                  {isFood 
                    ? 'Catalog photos shown to customers (up to 5)' 
                    : 'Primary images used for AI recognition (up to 5)'}
                </p>
              </div>
            ...
            {/* Raw Media Section (Clothing Only) */}
            {isClothing && (
